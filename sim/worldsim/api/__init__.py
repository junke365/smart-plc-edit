...
def list_scenarios():
    from sim.worldsim.scenarios.definitions import list_scenario_info
...
async def start_simulation(req: SimulationStartRequest):
    ...
    from sim.worldsim.scenarios.definitions import get_scenario
...
def get_metrics(sim_id: str):
    if sim_id not in _simulations:
        raise HTTPException(404, f"Simulation not found: {sim_id}")
    sim = _simulations[sim_id]
    from sim.worldsim.utils.metrics import MetricsCollector
    mc = MetricsCollector()
